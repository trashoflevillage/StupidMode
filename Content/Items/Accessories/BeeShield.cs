...
namespace StupidMode.Content.Items.Accessories
{
	...
	public class BeeDashPlayer : ModPlayer
	{
		...
		public override void PreUpdateMovement()
		{
			...
			{ // dash is active
			  ...
			  // Here we take advantage of "player.eocDash" and "player.armorEffectDrawShadowEOCShield" to get the Shield of Cthulhu's afterimage effect
				...
				if (Main.LocalPlayer == Player && Player.GetModPlayer<BeeDashPlayer>().DashTimer % 5 == 0 && Player.GetModPlayer<BeeDashPlayer>().DashTimer > 0)
				{
					if (Player.strongBees && Main.rand.NextBool())
					{
						Projectile.NewProjectile(Player.GetSource_FromAI(), Player.Center, Player.velocity * -0.5f, ProjectileID.GiantBee, 20, 0.5f, Player.whoAmI);
					}
					else
					{
						Projectile.NewProjectile(Player.GetSource_FromAI(), Player.Center, Player.velocity * -0.5f, ProjectileID.Bee, 10, 0, Player.whoAmI);
					}
				}
				// count down frames remaining
				DashTimer = (int)MathHelper.Clamp(DashTimer - 1, 0, 999999);
			}
		}

        private bool CanUseDash()
		{
			return BeeShieldEquipped
